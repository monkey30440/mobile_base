...
Localization 應作為定位狀態之唯一判定者，明確提供 `UNKNOWN`（可靠證據不足）、`HEALTHY`（定位可用）、`LOST`（可靠證據確認定位失效）。scan、TF 與有效量測新鮮度由 Localization 判定；Navigation 不得以狀態 topic 接收間隔重新推論定位狀態；但狀態發布來源失聯時，應停止信任快取狀態並中止導航，不得因此授權重定位。

導航僅可在 `HEALTHY` 時執行。`UNKNOWN` 應中止自主導航但不得觸發全域重定位；只有因明確 `LOST` 中止目前導航時，才允許單次全域重定位。Route、planner 或 controller failure 不得觸發全域重定位，且應保留原始錯誤傳播。

一旦確認 LOST，後續因證據不可用而變為 UNKNOWN 時，仍應保留尚未恢復的歷史；不可用期間不得計入恢復持續時間，重新取得有效資料後須重新連續達到恢復門檻。初始 UNKNOWN 的第一筆可靠高品質量測可直接建立 HEALTHY。

恢復成功必須取得重定位後新產生之 `HEALTHY` 量測證據，不能以等待時間結束判定成功。成功後應重新取得目前位姿並重建 route/path；有限恢復時間內仍無法取得健康證據則終止導航並回報定位失敗原因。定位可靠性門檻、持續時間、量測有效期及最大恢復時間需經實機驗證。
