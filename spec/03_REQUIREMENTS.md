...
導航期間，系統應持續判斷目前地圖定位是否仍足以支援安全導航；當定位持續無法可靠使用時，系統應停止目前自主導航運動並嘗試重新建立地圖定位。重新定位成功後，系統應使用恢復後之目前位姿重新建立後續導航；若在有限恢復時間內仍無法建立可靠定位，系統應終止導航並回報定位失敗原因。定位可靠性判定門檻、持續時間及最大恢復時間應由後續整合與驗證決定。
