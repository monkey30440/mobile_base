  ...
  const DynamicStageD1Options & options) noexcept
{
  ...
  if (std::isnan(options.left_wheel_cmd_rad_s) || std::isinf(options.left_wheel_cmd_rad_s)) {
    return {false, "Left wheel velocity command must be a finite number."};
  }
  if (options.left_wheel_cmd_rad_s <= 0.0) {
  ...
  if (options.warmup_cycles > STAGE_D1_MAX_WARMUP_CYCLES) {
    return {false, "Warm-up cycles exceeds maximum limit (" +
      std::to_string(STAGE_D1_MAX_WARMUP_CYCLES) + ")."};
  }
  if (options.active_cycles < 10 || options.active_cycles > STAGE_D1_MAX_ACTIVE_CYCLES) {
    return {false, "Active cycles must be in the range [10, " +
      std::to_string(STAGE_D1_MAX_ACTIVE_CYCLES) + "]."};
  }
  if (options.cooldown_cycles > STAGE_D1_MAX_COOLDOWN_CYCLES) {
    return {false, "Cooldown cycles exceeds maximum limit (" +
      std::to_string(STAGE_D1_MAX_COOLDOWN_CYCLES) + ")."};
  }
  return {true, ""};
}
...
DynamicStageD1Options parse_stage_d1_command_line(int argc, char ** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--rpm" || arg == "--right" || arg == "--right-vel" ||
      arg == "--both" || arg == "--reverse" || arg == "--linear" ||
      arg == "--angular" || arg == "--cmd_vel")
    {
      throw std::invalid_argument(
              "CRITICAL SAFETY VIOLATION: Argument (" + arg +
              ") is strictly prohibited. Stage D1 only exercises positive left wheel velocity.");
    }

    if ((arg == "--left-vel" || arg == "--left-velocity" || arg == "--left-rad-s") &&
      i + 1 < argc)
    {
      opts.left_wheel_cmd_rad_s = std::stod(argv[++i]);
...
static void print_stage_d1_plan(
  const DynamicStageD1Options & opts,
  std::ostream & out)
{
  const double left_rad_s = opts.left_wheel_cmd_rad_s;
  const int16_t left_rpm = M1Hardware::wheel_rad_s_to_motor_rpm(left_rad_s, 20.0, 1, 3000.0);
  const double linear_speed_m_s = left_rad_s * 0.08;
  const double expected_rot_rad = left_rad_s * (static_cast<double>(opts.active_cycles) / 30.0);
  const double expected_rot_deg = expected_rot_rad * 180.0 / M_PI;

  out << "================================================================\n"
      << "IMP-008 Level 4 Stage D1: Left Wheel Dynamic Feedback Plan\n"
      << "================================================================\n"
      ...
      << "Target Wheel          : LEFT WHEEL ONLY (Driver ID " << opts.driver_b << ", Sign +1)\n"
      << "Commanded Velocity    : " << left_rad_s << " rad/s (~"
      << std::fixed << std::setprecision(2) << linear_speed_m_s << " m/s surface speed)\n"
      << "Converted Target RPM  : " << left_rpm << " RPM (Left motor)\n"
      << "Right Wheel Target    : 0.0 rad/s (0 RPM, strictly hard-bound)\n"
...
int run_dynamic_stage_d1_check(
  ...
  std::ostream & err)
{
  const auto validation = validate_stage_d1_options(options);
  if (!validation.valid) {
    err << "Configuration Error: " << validation.error_message << "\n";
    return 1;
  }

  print_stage_d1_plan(options, out);
  ...
  const std::string urdf = build_stage_d1_urdf(options);
  auto clock = std::make_shared<rclcpp::Clock>(RCL_ROS_TIME);
  auto logger = rclcpp::get_logger("m1_dynamic_stage_d1_check");
  ...
  std::vector<DynamicStageD1Sample> samples;
  samples.reserve(total_cycles);

  out << "[Step 1] Executing Stage D1 dynamic motion loop ("
      << options.warmup_cycles << " warmup + "
      << options.active_cycles << " active @ " << options.left_wheel_cmd_rad_s << " rad/s + "
  ...
  for (size_t i = 0; i < total_cycles; ++i) {
    ...
    if (i >= options.warmup_cycles && i < options.warmup_cycles + options.active_cycles) {
      phase = "active";
      left_cmd = options.left_wheel_cmd_rad_s;
    } else if (i >= options.warmup_cycles + options.active_cycles) {
      phase = "cooldown";
      left_cmd = 0.0;
    }

    // Set command interfaces (Right wheel is strictly 0.0)
    static_cast<void>(cmd_left.set_value(left_cmd));
    static_cast<void>(cmd_right.set_value(0.0));
    ...
    DynamicStageD1Sample sample;
    sample.seq = i + 1;
    sample.phase = phase;
    sample.cycle_duration_us = cycle_duration_us;
    sample.left_cmd_rad_s = left_cmd;
    sample.right_cmd_rad_s = 0.0;
    sample.left_target_rpm = M1Hardware::wheel_rad_s_to_motor_rpm(left_cmd, 20.0, 1, 3000.0);
    sample.right_target_rpm = 0;
    ...
    // Anomaly checks
    if (phase == "active") {
      if (sample.right_wheel_vel_rad_s > 0.2 || sample.right_wheel_vel_rad_s < -0.2) {
        abort_triggered = true;
        abort_reason = "Unexpected right wheel motion observed during left-only test";
        break;
      }
      if (sample.left_wheel_vel_rad_s < -0.05) {
        abort_triggered = true;
        abort_reason = "Wrong left wheel rotation direction (negative velocity detected)";
        break;
      }
    }
  ...
  if (abort_triggered) {
    err << "CRITICAL ERROR: Stage D1 aborted: " << abort_reason << "\n";
    return 1;
  }
  ...
  if (!samples.empty()) {
    ...
    out << "\n================================================================\n"
        << "       STAGE D1 LEFT WHEEL DYNAMIC FEEDBACK RESULTS             \n"
        << "================================================================\n"
        << "Total Cycles Executed : " << samples.size() << "\n"
        << "Left Pos Start / End  : " << std::fixed << std::setprecision(4)
        << first.left_wheel_pos_rad << " rad -> " << last.left_wheel_pos_rad << " rad\n"
        << "Left Pos Delta        : " << delta_left_rad << " rad ("
        << (delta_left_rad * 180.0 / M_PI) << " deg)\n"
        << "Right Pos Start / End : " << first.right_wheel_pos_rad << " rad -> "
        << last.right_wheel_pos_rad << " rad\n"
        << "Right Pos Delta       : " << delta_right_rad << " rad\n"
        << "Direction Consistency : " << (delta_left_rad > 0.0 ? "PASS (Positive)" : "FAIL") << "\n"
        << "Isolation Consistency : " << (std::abs(delta_right_rad) <
    0.05 ? "PASS (Zero)" : "FAIL") << "\n"
        << "================================================================\n";
  }
