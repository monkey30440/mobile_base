...
1. **Phase & Architecture Evidence Reports ([`docs/evidence/`](file:///home/jim/mobile_base/docs/evidence/)):** 17 structured technical reports covering runtime closures, hardware preflight, controlled motion characterization, scan decoupling, launch entry optimization, and baseline investigations.
...
The following table catalogs all 17 files in [`docs/evidence/`](file:///home/jim/mobile_base/docs/evidence/):

| ID | Path | Classification | Evidence Type | Scope | Date / Commit | Current Use | Limitations |
|---|---|---|---|---|---|---|---|
| **EVID-E01** | [`baseline_reset_audit_report.txt`](file:///home/jim/mobile_base/docs/evidence/baseline_reset_audit_report.txt) | **INVESTIGATION-ONLY** | Audit / Investigation Report | System odometry transition (RF2O → Kinematic-ICP), package inventory, and launch topology comparison | 2026-08-26<br>`f0de34e` | Documents architectural context and rationale for Kinematic-ICP baseline reset | Investigation report; does not provide direct execution telemetry |
| **EVID-E02** | [`discovery_report.txt`](file:///home/jim/mobile_base/docs/evidence/discovery_report.txt) | **INVESTIGATION-ONLY** | Discovery / Inventory Report | Initial repository inventory, file counts, mesh asset sizes, preliminary doc discrepancies at commit `9028a58` | 2026-08-26<br>`f0de34e` | Historical snapshot of repository state prior to documentation rework | Exploratory audit; non-normative |
...
| **EVID-E04** | [`phase_cm_f1_collision_scan_self_filter_report.txt`](file:///home/jim/mobile_base/docs/evidence/phase_cm_f1_collision_scan_self_filter_report.txt) | **SUPERSEDED** | Implementation & Verification Report | Intermediate collision scan self-filter implementation using `laser_filters` (`LaserScanBoxFilter`) and `/scan_collision` | 2026-08-26<br>`f0de34e` | Historical record of intermediate self-filter attempt | Superseded by scan decoupling architecture ([`scan_decoupling_report.txt`](file:///home/jim/mobile_base/docs/evidence/scan_decoupling_report.txt) / `4ed33f2`), where `laser_filters` was removed in favor of direct dual-scan collision monitoring |
...
| **EVID-E12** | [`release_guide.txt`](file:///home/jim/mobile_base/docs/evidence/release_guide.txt) | **INVESTIGATION-ONLY** | Operational / Packaging Guide | Multi-stage Docker binary release packaging workflow and `export_release.sh` usage | 2026-08-27<br>`2faa590` | Operational reference for generating protected binary release packages | Operational guide, not a verification artifact |
| **EVID-E13** | [`release_prompt_template.txt`](file:///home/jim/mobile_base/docs/evidence/release_prompt_template.txt) | **INVESTIGATION-ONLY** | Design Prompt Template | Generic ROS 2 binary packaging and source-code protection prompt template | 2026-08-27<br>`2faa590` | Reference prompt template | Template artifact; non-normative |
...
| **EVID-E15** | [`station_id_navigation_design_investigation.txt`](file:///home/jim/mobile_base/docs/evidence/station_id_navigation_design_investigation.txt) | **INVESTIGATION-ONLY** | Design Investigation Note | Architectural trade-off analysis comparing ROS Action server vs thin C++ CLI wrapper for Station ID navigation (selecting Model B CLI wrapper) | 2026-08-26<br>`f0de34e` | Documents design rationale for CLI-based `navigate_to_station` | Pre-implementation analysis; non-executable |
